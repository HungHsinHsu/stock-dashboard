...
def _render_entry_gates(code, df, last, ma5, ma60):
    """個股進場四關：依現價實時判定，一關一關打勾/打叉，並顯示分級結論。
    用的是與每日預測同一套規則(core.rules.entry_setup)，所以與訊號一致。
    分級（中間版）：①到位②站穩＝必要；④外資賣超＝否決；③量縮＋④停手計分定強度。"""
    st.markdown("##### 🚪 進場四關（依現價實時判定，一關一關看）")
    st.caption("①②為必要門檻、④外資賣超會否決；③量縮＋④外資停手是「加分」，"
               "決定進場強度（半批試單→標準→強）。不必四關全過。")
    ...
    st.table(pd.DataFrame([
        {"關卡": "① 價格到位（回到支撐±2%）〔必要〕", "現況": g1},
        {"關卡": "② 收盤站穩（沒再破底）〔必要・防接刀〕", "現況": g2},
        {"關卡": "③ 量縮（量比<1＝賣壓衰竭）〔加分〕", "現況": g3},
        {"關卡": "④ 外資（賣超則否決／停手加分）", "現況": g4},
    ]).set_index("關卡"))

    tier = setup.get("tier", ceiling)
    if ceiling == "進場":
        st.success(f"✅ 可進場 → **{tier}**（進「{at_batch or '下一批'}」，"
                   "盤後定價 14:00–14:30）。分數越高（量縮＋外資停手）越可放大到標準/加碼；"
                   "只過必要門檻則先半批試單。")
    elif ceiling == "避開":
        st.error(f"🛑 {tier}：{setup['reason']}")
    else:
        st.warning(f"⏳ {tier}——{setup['reason']}。①②是必要門檻、④外資賣超會否決；"
                   "先等必要關打勾，再看③④加分定進場強度。")
    ...
    if is_etf(code):
        ...
        scen = [
            ("high", "在支撐1之上（位置偏高、還沒回檔）",
             "觀望：不追高，等回檔到支撐。錯過無傷。"),
            ("b1", "回到支撐1附近",
             "第一批：站穩才進（必要）；量縮＋外資停手越多分，越能從半批試單放大到標準。"),
            ("vac", "支撐與支撐之間的真空帶",
             "等：不上不下不進場，等跌到下一段支撐、或帶量站回上一條支撐。"),
            ("b2", "回到支撐2附近",
             "第二批：站穩才進（必要）；再看量縮＋外資停手加分定強度（半批試單→標準）。"),
            ("b3", "回到支撐3附近",
             "第三批（最後一批）：站穩才進（必要）；量縮＋外資停手加分定強度。"),
            ("stop", "收盤跌破支撐3",
             "停損區：三批全數認賠出場，不接刀。"),
        ]
