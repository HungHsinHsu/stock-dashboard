...
def entry_setup(ind, code=None, foreign_stopped=None):
    ...
    if d3 is not None and d3 < 0:
        ...
        # 季線高過月線＝60 日窗口裡還塞著崩跌前的舊高，這條線問的是「有沒有漲回舊高」，
        # 不是「趨勢好不好」。上限維持避開（門檻一格都不放寬），但別再叫人去盯一條被
        # 舊高墊高的線——那會把「還沒漲回前高」讀成「趨勢還沒好」，兩者對使用者的意義相反。
        # 實例（2026-08-13 同一天三檔）：00735 收 103.6／MA20 95.89／MA60 105.53、
        # 世界先進 163.0／154.80／169.95、廣達 325.0／312.73／346.68——季線都高過月線約 10%，
        # 但三檔都是價 > MA5 > MA20 的多頭排列，是崩跌後修復，不是轉空。
        if isinstance(ma60, (int, float)) and ma20 and ma60 > ma20:
            return result("避開", None,
                          f"收盤仍在{lv}之下{chg}，但{lv}高過月線 {ma20:.1f}"
                          "（60 日窗口含崩跌前舊高）→ 這是『還沒漲回前高』不是趨勢轉空；"
                          "仍不買進，但別等季線，改盯月線斜率(ma20_slope5)翻正"
